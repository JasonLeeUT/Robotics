...
%
% Code very slightly adapted by Marc Freese (line 167 & 196)
...
% Following line modified by Marc Freese:
    str = char(bytes(idx:idx+len-1)'); % str = native2unicode(bytes(idx:idx+len-1)', 'utf-8');
...
function [out, idx] = parsemap(len, bytes, idx)
    ...
    for n=1:len
        ...
        % Following 3 added by Marc Freese:
        if not(ischar(key)) 
            key=transpose(char(key));
        end
